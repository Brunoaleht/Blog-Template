...
export default function CategoryPage({ posts, setting, variable }: PostsTemplateProps) {
  const router = useRouter();
  if (router.isFallback) {
    return <h1>Loading</h1>;
  }
  const categoryName = posts.postData[0].attributes.categories.categoryData.filter(
  ...
  return (
    <>
      <Head>
        <title>
          Category: {categoryName} - {setting.settingData.attributes.blogName}
        </title>
